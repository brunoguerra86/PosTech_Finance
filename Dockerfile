...
COPY --from=build /app/target/*.jar /app/msfinance.jar

#CMD ["java", "-jar", "/app/msfinance.jar"]
...
# Aguardando até que o MongoDB inicie antes de iniciar a aplicação
CMD ["/bin/bash", "-c", "wait-for-it.sh -t 0 mongo-finance:27017 -- java -jar msfinance.jar"]